...
function addGamesToPage(games) {
    ...
    // loop over each item in the data
        ...
        // between the end of the src attribute and the end of the tag ("/>")
            game_card.innerHTML = `<h1>${game['name']}</h1> <p>${game.description}</p> <img src = "${game.img}" class = "game-img">`
...
let funding = GAMES_JSON.reduce( (acc, game) => {
    return acc + game.pledged;
}, 0);

// create a string that explains the number of unfunded games using the ternary operator
const displayStr = `A total of $${funding.toLocaleString('en-US')} has been raised for ${GAMES_JSON.length} games. Currently, ${countUnfundedGames == 1 ? '1 game remains unfunded. ' : countUnfundedGames+' games remain unfunded. '}We need your help to fund these amazing games!`

// create a new DOM element containing the template string and append it to the description container
const str = document.createElement('p');
str.textContent = displayStr;
